    ...
    private void charAlignSubsequences() {
        ...
        for ( int i = 0; i <= alignment.size(); i++ ) {
            ...
            if ( cand != null && ref != null ) {
                if (cand.equals(ref)){ // matching words?
                    // end of the accumulation reached.                   
                    if  (refAccuSize == 0){
                        if ( candAccuSize != 0){
                            // add candAcc characters into the aligned sequence
                            for(int kk = 0; kk < candAccuSize; kk++){
                                alignment2.add(new AlignedSequence(null, candAccu[kk]));
                            } 
                        }
                    }else{
                        if ( candAccuSize == 0 ) {
                            // add refAccu characters into the aligned sequence
                            for(int kk = 0; kk < refAccuSize ; kk++){
                                alignment2.add(new AlignedSequence(refAccu[kk],null));
                            } 
                        }else{ // that means there are two sequences to align
                            
                            // TODO: aligning char sequences might be done faster
                            
                            // align char subsequences
                           if ( ((long) refAccuSize * (long)candAccuSize ) < MAX_DYNAMIC_TABLE_SIZE ){
                                ...
                                // sequences are too large to be aligned using dynamic programming (very slow).
                                // then one may use simple heuristic to align chars.
                                // or simply print them out without aligning them.
                                for (int ls = 0; ls < refAccuSize; ls++){
                                    alignment2.add(new AlignedSequence(refAccu[ls],null));
                                }
                                for (int ls = 0; ls < candAccuSize; ls++) {
                                    alignment2.add(new AlignedSequence(null, candAccu[ls]));
                                }
                            } 
                        }
                    }
                    
                    // we already know that current words align
                    for ( int j = 0 ; j < ref.length(); j++){
                        String ch = ref.substring(j, j+1);
                        alignment2.add(new AlignedSequence(ch, ch));
                    }
                    //alignment2.add(new AlignedSequence(WORD_BOUNDARY, WORD_BOUNDARY));
                    ...
                    for ( int ll = 0; ll < ref.length(); ll++){
                        refAccu[refAccuSize] = ref.substring(ll, ll+1);
                        refAccuSize++;
                    }
                    refAccu[refAccuSize++] = " ";

                    for ( int ll = 0; ll < cand.length(); ll++){
                        candAccu[candAccuSize] = cand.substring(ll, ll+1);
                        candAccuSize++;
                    }
                    candAccu[candAccuSize++] = " ";
                }
            }else{
                if ( cand == null ){
                    for ( int ll = 0; ll < ref.length(); ll++){
                ...
                }else{ // that means cur.m_reference is null
                    for ( int ll = 0; ll < cand.length(); ll++){
                        candAccu[candAccuSize] = cand.substring(ll, ll+1);
                        candAccuSize++;
                    }
  ...
  /*      if ( REPORT ){
    ...
    public Stats calculateOCRaccuracy() {
        ...
        if (alignment.isEmpty()) {
            System.out.println("There are no words aligned. Aligner must be run prior to calling calculateOCRaccuracy");
            return null;
        }
   ...
   //     System.out.println("OCR accuracy: " + wordAccuracy + " (matching words= " + (total - numError) + " reference length= " + total + ")");
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException {
        ...
        String USAGE ="\nRecursive Text Alignment Tool (RETAS V1.0) Copyright (C) 2013 by the University of Massachusetts at Amherst\n" +
                ...
                + "\n\n TRY: java RecursiveAlignmentTool texts/adventuresofhuck_gutenberg.txt texts/adventuresofhuck00clemrich.txt alignmentOutput.txt -opt config.txt";
     ...
     * @param candFile  input text 2: OCR output text (or the candidate text)
     * @param ignoredChars  The list of characters to be ignored
     * @return It returns a Stats array of size two. The first element includes the statistics for the word level alignment (i.e., number of matching words, the length of the sequences aligned). The second element gives the  statistics for the character level alignment    
     */ 
